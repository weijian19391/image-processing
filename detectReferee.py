...
THRESHOLD = {"Red":80, "Blue":40, "Green":150, "Black":0}
...
BOUNDARIES_FOR_BLACKSHORTS = [([0,20,17],[40,90,75])] #can detect full body of referee sometimes
...
def detectColour(frame, colour, threshold):
    ...
    if colour != "Black":
        ...
        for(lower, upper) in BOUNDARIES_FOR_BLACKSHORTS:
            ...
            masked_unwanted_points = np.ma.masked_where(output!=0, output)
            max_intensity_pixels = np.ma.filled(masked_unwanted_points,255)
...
if __name__ == "__main__":
    ...
    while capture.isOpened():
        ...
        #cv2.imwrite("Images\Players in white frame"+str(j)+ ".png",final_full_all_players_white)
        ...
        cv2.imwrite("Images\Added_referee " + "frame "+ str(j)+ ".png",full_referee) #gets full image of referee but too noisy
        ...
        # final_full_green_player = np.ma.filled(masked_wanted_points,255)
        #
        # cv2.imwrite("detect " +"Green " + "frame "+ str(j)+ ".png",final_full_green_player)
